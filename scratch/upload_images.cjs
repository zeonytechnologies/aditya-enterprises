...
const imgDir = 'd:\\\\ZEONY\\\\ADITYA ENTERPRISES\\\\88 Images Pidilite';
const brand_id = 'b2222222-2222-2222-2222-222222222222';
const bucketName = 'products'; // Trying products as default bucket name if product-images fails

async function main() {
  const files = fs.readdirSync(imgDir);
  let updatedCount = 0;
  
  // First, check if 'products' bucket exists, if not, try to create or just use 'images'
  ...
  for (const file of files) {
    ...
    const uploadPath = `pidilite/${file}`;
