...
const priceMap = JSON.parse(fs.readFileSync('price_map_3.json', 'utf8'));
...
async function generateSQL() {
  ...
  let sql = '-- Auto-generated price updates from Dealer PDF\n\n';
  ...
  fs.writeFileSync('update_prices_dealer_pdf.sql', sql);
  console.log(`Generated update_prices_dealer_pdf.sql with ${sql.split('\n').length - 2} statements.`);
}
